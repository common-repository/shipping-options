 ...
 *
 * @version        1.0.0
 * @package        shipping-options/modules
 * @author        impleCode
 */
...
function shipping_menu() {
	if ( current_user_can( 'manage_product_settings' ) ) {
        ...
        <a id="shipping-settings" class="nav-tab"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=shipping-settings&submenu=shipping' ) ?>"><?php _e( 'Shipping', 'shipping-options' ); ?></a>
		<?php
	}
}
...
function shipping_settings_content() {
    ...
    <h3>
        <a id="shipping-settings" class="element current"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=shipping-settings&submenu=shipping' ) ?>"><?php _e( 'Shipping Settings', 'shipping-options' ); ?></a>
        ...
        <div class="setting-content submenu">
            ...
            <h2><?php _e( 'Shipping Settings', 'shipping-options' ); ?></h2>
            <form method="post" action="options.php">
				<?php
				settings_fields( 'product_shipping' );
				$shipping_count = get_shipping_options_number();
				?>
                <h3><?php _e( 'Shipping options', 'shipping-options' ); ?></h3>
                <table>
                    <tr>
                        <td colspan="2"><?php _e( 'Number of shipping options', 'shipping-options' ); ?> <input
                                    ...
                                    id="admin-number-field" value="<?php echo $shipping_count; ?>"/><input type="submit"
                                                                                                           class="button"
                                                                                                           value="<?php _e( 'Update', 'shipping-options' ); ?>"/>
                        </td>
                    </tr>
                </table>
				<?php
				ic_register_setting( __( 'Number of shipping options', 'shipping-options' ), 'product_shipping_options_number' );
				if ( $shipping_count > 0 ) {
					?>
                    <div class="al-box info">
                        ...
                        <p><?php _e( "When every item in your catalogue has different shipping options you can leave all or just a part of these fields empty.", 'shipping-options' ) ?></p>
                        <p><?php _e( 'You can change these default values for every new item in the catalog.', 'shipping-options' ); ?></p>
                    </div>
                    <div class="settings-table-container" style="overflow-x: scroll;">
                        <table class="wp-list-table widefat product-settings-table dragable">
                            ...
                            <tr>
                                <th></th>
                                <th class="title">
                                    <b><?php _e( 'Shipping default name', 'shipping-options' ); ?></b></th>
                                <th></th>
                                <th class="title">
                                    <b><?php _e( 'Shipping default cost', 'shipping-options' ); ?></b></th>
                                <th class="dragger"></th>
                            </tr>
                            </thead>
                            <tbody>
                    ...
                    <p class="submit">
                        <input type="submit" class="button-primary"
                               value="<?php _e( 'Save changes', 'shipping-options' ); ?>"/>
                    </p>
				<?php } else { ?>
                    <tr>
                        <td colspan="2">
                            <div class="al-box warning"><?php _e( 'Shipping disabled. To enable set minimum 1 shipping option.', 'shipping-options' ); ?></div>
                        </td>
                    </tr>
                    </table>
				<?php } ?>
...
function ic_shipping_single_names( $single_names ) {
	implecode_settings_text( __( 'Shipping Label', 'shipping-options' ), 'single_names[product_shipping]', $single_names['product_shipping'] );
	implecode_settings_text( __( 'Free Shipping Text', 'shipping-options' ), 'single_names[free_shipping]', $single_names['free_shipping'] );
}
